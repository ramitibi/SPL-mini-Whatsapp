...
public class ConnectionHandler<T> {
	 ...
	 */
	private ConnectionHandler(SocketChannel sChannel, ReactorData<T> data, SelectionKey key, WhatsappProtocol manger) {
		_sChannel = sChannel;
		_data = data;
		_protocol = _data.getProtocolMaker().create(manger);
	...
	public static <T> ConnectionHandler<T> create(SocketChannel sChannel, ReactorData<T> data, SelectionKey key, WhatsappProtocol manger) {
		ConnectionHandler<T> h = new ConnectionHandler<T>(sChannel, data, key, manger);
		h.initialize();
		return h;
	}
